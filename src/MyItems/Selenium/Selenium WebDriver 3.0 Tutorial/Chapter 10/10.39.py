...
class IdentifyNewPopUpWindowByTitle(unittest.TestCase):
    def setUp(self):
        self.driver = Browser.Chrome()

    def testIdentifyNewPopUpWindowByTitle(self):
        driver = self.driver
            ...
            try:
                for WindowHandle in All_Handles:
                    ...
                    if '百度一下，你就知道' == driver.title:
                        WebDriverWait(driver,10,0.2).until(lambda x:x.find_element_by_id('kw')).send_keys('百度首页的窗口被找到了!') #WebDriverWait是设置显式等待的方法,这里设置10秒等待时间,并找到百度搜索框，输入所期望的内容
                        time.sleep(2)
