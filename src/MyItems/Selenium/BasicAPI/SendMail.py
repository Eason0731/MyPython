...
def Send_Mail_by_QQ(Latest_Report_Path): #发送邮件方法
    #-----------1.跟发件相关的参数------
    smtpserver = 'smtp.qq.com' #发件服务器
    port = 465 #端口
    username = 'lonlon29@qq.com' #发件箱用户名
    password = 'uvwugrwppapdcahd' #发件箱密码
...
def Send_Mail_by_Sina(Latest_Report_Path): #发送邮件方法
    #-----------1.跟发件相关的参数------
    smtpserver = 'smtp.sina.cn' #发件服务器
    port = 25 #端口
    username = 'lonlon29@sina.cn' #发件箱用户名
    password = '2e059d17a6db438f' #发件箱密码
    ...
    ##发送邮件
    smtp = smtplib.SMTP()
    smtp.connect('smtp.sina.cn') # 邮箱服务器
    smtp.login(username, password) # 登录邮箱
    smtp.sendmail(sender, receiver, msg.as_string()) # 发送者和接收者
    smtp.quit()
